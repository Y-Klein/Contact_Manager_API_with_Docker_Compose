...
def get_all_contacts():
    with connection.cursor() as cursor:
        ...
        for my_object in all_contacts:
            result.append(Contact.get_dict(Contact(
                id=my_object[0],
                first_name=my_object[1],
                last_name=my_object[2],
                phone_number=my_object[3]
                )))
        return result
...
def delete_contact(my_id):
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM contacts WHERE id = %s",[my_id])
        connection.commit()
        return True
